...
import {ADDRESS_REWARDTOKEN, ADDRESS_SATIN, ADDRESS_ZERO, ZERO_BD} from './constants';
...
export function handleSetVeDist(event: SetVeDist): void {
  ...
  log.info("ve address {}",[veDistCtr.votingEscrow().toHexString()])
...
function createVe(veAdr: string): void {
  ...
  if (!ve) {
    ...
    ve.underlying = ADDRESS_REWARDTOKEN;
    
    const mintable = MintableAbi.bind(Address.fromString(ADDRESS_SATIN))
